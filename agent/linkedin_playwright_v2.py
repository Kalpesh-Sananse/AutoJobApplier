...
class LinkedInBotPlaywright:
    ...
    async def _handle_application_modal(self, job_index=0):
        ...
        max_attempts = 30  # Increased for multi-page forms
        attempt = 0
        page_num = 1
        ...
        while attempt < max_attempts:
            ...
            await self.page.screenshot(path=f"job_{job_index}_page_{page_num}_step_{attempt}.png")
            ...
            if next_btn:
                page_num += 1
                self.logger.info(f"Clicking Next to page {page_num}...")
        ...
        self.logger.warning(f"Could not complete application after {max_attempts} attempts")
        await self.page.screenshot(path=f"job_{job_index}_timeout.png")
        await self._close_modal()
        return False

    async def _fill_form_fields(self):
        """Fill all form fields using AI - improved version."""
        
        # Text inputs - with better error handling
        ...
        for inp in text_inputs:
            try:
                ...
                answer = self.ai_handler.generate_answer(label_text, field_type=field_type)
                if answer:
                    ...
                    try:
                        ...
                        # Field might have disappeared or become readonly
                        pass
            except Exception as e:
        ...
        for fieldset in fieldsets:
            try:
                ...
                choice = self.ai_handler.generate_answer(question, field_type="text")
        ...
        for select in selects:
            try:
                ...
                if len(options) > 1:
                    question = f"{label_text} Options: {', '.join(options)}"
                    choice = self.ai_handler.generate_answer(question, field_type="text")
